...
const addTutorContent = (tutorIdentity, tutorEmail, learnerIdentity, courseName, gSheetId, subProgram) => {
  const VAEISubprogramIds = UtilsHelper.getEnvObjectIds('VAEI_SUBPROGRAM_IDS');
  const PRISubprogramIds = UtilsHelper.getEnvObjectIds('PRI_SUBPROGRAM_IDS');
  const isSubProgramVAEI = UtilsHelper.doesArrayIncludeId(VAEISubprogramIds, subProgram);
  const isSubProgramPRI = UtilsHelper.doesArrayIncludeId(PRISubprogramIds, subProgram);
  const lastSessionTiming = isSubProgramVAEI ? 'mois 10 environ' : 'mois 4 ou 5';
  const courseVerbatim = isSubProgramVAEI ? 'VAEI' : 'PRI';

  return `
    <p>Bonjour ${tutorIdentity},</p>
    ...
    ? `<p>Pour information, la formation dédiée aux tuteurs.trices est <strong>obligatoire</strong> et est divisée en trois sessions :</p>
      <ul>
        <li><strong>Session 1 (mois 1 ou 2) :</strong> Présentation de la formation ${courseVerbatim} et des outils du tuteur (1h30)</li>
        <li><strong>Session 2 (mois 3 environ) :</strong> Rôle tuteur.trice dans l’accompagnement${isSubProgramPRI ? ', l\'évaluation en situation' : ''} (1h30)</li>
        <li>Réalisation en autonomie de trois modules e-learning (4h00)</li>
        <li><strong>Session 3 (${lastSessionTiming}) :</strong> Accompagner la préparation au passage de la VAE Aide-Soignant (1h00)</li>
      </ul>
