...
exports.authorizeCourseBillListCreation = async (req) => {
  ...
  const course = await Course
    .findOne({ _id: courseId }, { type: 1, expectedBillsCount: 1, companies: 1, prices: 1, interruptionDates: 1 })
    .lean();

  if (!course) throw Boom.notFound();

  const isCourseInterrupted = UtilsHelper.isCourseInterrupted(course.interruptionDates);
  if (isCourseInterrupted) throw Boom.forbidden();
...
exports.authorizeCourseBillingPurchaseUpdate = async (req) => {
  ...
  const isTrainerFeesWithPercentage = has(purchase, 'percentage') &&
    UtilsHelper.areObjectIdsEquals(purchase.billingItem, process.env.MANAGEMENT_FEES_BILLING_ITEM);
...
exports.authorizeCourseBillingPurchaseDelete = async (req) => {
  ...
  const isTrainerFeesWithPercentage = purchaseRelatedToBill.percentage &&
    UtilsHelper.areObjectIdsEquals(purchaseRelatedToBill.billingItem, process.env.MANAGEMENT_FEES_BILLING_ITEM);
