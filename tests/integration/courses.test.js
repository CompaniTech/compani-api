...
describe('COURSES ROUTES - PUT /courses/{_id}', () => {
  ...
  describe('TRAINING_ORGANISATION_MANAGER', () => {
    ...
    it('should update course (intra)', async () => {
      const payload = {
        misc: 'new name',
    ...
    it('should update company representative on single course', async () => {
    ...
    it('should return 403 if try to add company representative to inter_b2b course', async () => {
      const payload = { companyRepresentative: holdingAdminFromOtherCompany._id };
      const response = await app.inject({
        method: 'PUT',
        url: `/courses/${coursesList[4]._id}`,
...
describe('COURSES ROUTES - PUT /courses/{_id}/trainers/{trainerId}', () => {
  let authToken;
...
describe('COURSES ROUTES - PUT /courses/{_id}/trainees-csv', () => {
  ...
  describe('TRAINING_ORGANISATION_MANAGER', () => {
    ...
    it('should create and add trainees to course', async () => {
    ...
    it('should return 404 if course doesn\'t exist', async () => {
      ...
      const response = await app.inject({
        method: 'PUT',
        url: `/courses/${new ObjectId()}/trainees-csv`,
        headers: { ...form.getHeaders(), Cookie: `${process.env.ALENVI_TOKEN}=${authToken}` },
        payload: getStream(form),
      });
...
describe('COURSES ROUTES - POST /courses/single-courses-csv', () => {
  ...
  describe('TRAINING_ORGANISATION_MANAGER', () => {
    ...
    it('should create courses', async () => {
      const usersBefore = await User.countDocuments();
      const userCompaniesBefore = await UserCompany.countDocuments();
      ...
      parseCSV.returns([
        ...
        },
        {
          firstname: 'Salut',
          lastname: 'Toi',
          email: 'traineeWithoutCompany@alenvi.io',
          countryCode: '',
          phone: '0687654321',
          company: 'Nouvelle Structure',
          suffix: '@test.fr',
      ...
      const usersAfter = await User.countDocuments();
      expect(usersAfter).toEqual(usersBefore + 1);
      const userCompaniesAfter = await UserCompany.countDocuments();
      expect(userCompaniesAfter).toEqual(userCompaniesBefore + 2);
      ...
      sinon.assert.callCount(gsheetsWriteData, 8);
    });
    ...
    it('should return 422 if coach has wrong role', async () => {
      const formData = { file: 'test' };
      const form = generateFormData(formData);
      ...
      expect(response.statusCode).toBe(422);
      expect(Object.values(response.result.errorsByTrainee)[0]).toEqual(['le coach n\'existe pas']);
    });

    it('should return 422 if coach email is incorrect', async () => {
      const formData = { file: 'test' };
      const form = generateFormData(formData);
      ...
      expect(response.statusCode).toBe(422);
      expect(Object.values(response.result.errorsByTrainee)[0])
        .toEqual(['le format de l\'email est incorrect pour le coach']);
    });

    it('should return 422 if architect has wrong role', async () => {
      const formData = { file: 'test' };
      const form = generateFormData(formData);
      ...
      expect(response.statusCode).toBe(422);
      expect(Object.values(response.result.errorsByTrainee)[0]).toEqual(['l\'architecte n\'existe pas']);
    });

    it('should return 422 if architect email is incorrect', async () => {
      const formData = { file: 'test' };
      const form = generateFormData(formData);
