...
describe('ATTENDANCE SHEETS ROUTES - POST /attendancesheets', () => {
  ...
  describe('TRAINER', () => {
    ...
    it('should return 403 if try to add attendance sheet on another trainer slot (inter)', async () => {
      const slots = [slotsList[26]._id.toHexString()];
      const formData = {
        course: coursesList[1]._id.toHexString(),
        signature: 'test',
        trainees: userList[2]._id.toHexString(),
        origin: MOBILE,
        trainer: trainer._id.toHexString(),
      };
...
describe('ATTENDANCE SHEETS ROUTES - PUT /attendancesheets/{_id}', () => {
  ...
  describe('TRAINER', () => {
    ...
    it('should generate attendance sheet file for intra holding course', async () => {
    ...
    it('should return 403 if try to add slot linked to another trainer', async () => {
      const attendanceSheetId = attendanceSheetList[5]._id;
      const payload = { slots: [slotsList[28]._id] };
...
describe('ATTENDANCE SHEETS ROUTES - DELETE /attendancesheets/{_id}', () => {
  ...
  describe('TRAINER', () => {
    ...
    it('should return 403 if trainer tries to delete another trainer\'s attendance sheet (co-intervention)',
      async () => {
        const response = await app.inject({
          method: 'DELETE',
          url: `/attendancesheets/${attendanceSheetList[16]._id}`,
          headers: { Cookie: `${process.env.ALENVI_TOKEN}=${authToken}` },
        });
    ...
    it('should return 403 if try to delete attendance sheet and attendances linked to trainer bill', async () => {
      const attendanceSheetId = attendanceSheetList[15]._id;
      const response = await app.inject({
        method: 'DELETE',
        url: `/attendancesheets/${attendanceSheetId}?shouldDeleteAttendances=true`,
        headers: { Cookie: `${process.env.ALENVI_TOKEN}=${authToken}` },
