...
describe('list', () => {
  ...
  it('should return all company bills (with credit note) (vendor role)', async () => {
    ...
    expect(result).toEqual([{
      course: { _id: courseId, misc: 'group 1', tradeName: 'program 1' },
      company: companyId,
      mainFee: { price: 120, count: 2 },
      payer: { company: companyId },
      billingPurchaseList: [
  ...
  it('should return all draft course bills between two dates', async () => {
    ...
    SinonMongoose.calledOnceWithExactly(
      subProgramFind,
      [
        { query: 'find', args: [{ _id: { $in: [singleSubProgramId.toHexString()] } }] },
        { query: 'populate', args: [{ path: 'steps', select: 'activities' }] },
        { query: 'lean' },
      ]
    );
    SinonMongoose.calledOnceWithExactly(
...
describe('updateCourseBill', () => {
  ...
  it('should invoice bill', async () => {
    ...
    const lastPaymentNumber = { seq: 2 };
    const courseBill = {
      _id: courseBillId,
      mainFee: { price: 200, percentage: 40, count: 1 },
      companies: [companyId],
      billedAt: '2022-03-08T00:00:00.000Z',
      billingPurchaseList: [{ billingItem: MANAGEMENT_FEES_BILLING_ITEM, price: 20, percentage: 40, count: 1 }],
      course: { type: SINGLE },
    };
    ...
    SinonMongoose.calledOnceWithExactly(
      findOneAndUpdateCoursePaymentNumber,
      [
        {
          query: 'findOneAndUpdate',
...
describe('updateBillList', () => {
  ...
  let findOneAndUpdateCourseBill;
  ...
  beforeEach(() => {
    ...
    findOneAndUpdateCourseBill = sinon.stub(CourseBill, 'findOneAndUpdate');
  ...
  afterEach(() => {
    ...
    findOneAndUpdateCourseBill.restore();
  ...
  it('should invoice bills', async () => {
    ...
    SinonMongoose.calledOnceWithExactly(
      ...
      [
        {
          query: 'findOneAndUpdate',
          args: [{ nature: PAYMENT }, { $inc: { seq: 2 } }, { new: true, upsert: true, setDefaultsOnInsert: true }],
        },
        { query: 'lean' },
      ]
    );
    sinon.assert.calledWithExactly(
      coursePaymentCreate.getCall(0),
      {
        companies: [companyId],
        number: 'REG-00002',
        status: PENDING,
        netInclTaxes: 200,
        nature: PAYMENT,
        courseBill: courseBillIds[0],
        date: '2022-03-08T00:00:00.000Z',
        type: BANK_TRANSFER,
      }
    );
    sinon.assert.calledWithExactly(
      coursePaymentCreate.getCall(1),
      {
        companies: [companyId],
        number: 'REG-00003',
        status: PENDING,
