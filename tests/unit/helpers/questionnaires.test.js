...
describe('list', () => {
  ...
  it('should not return self-positionning questionnaire for end course if course not ended (not connected)',
    async () => {
      const courseId = new ObjectId();
      const programId = new ObjectId();
      const questionnaires = [
      ...
      const result = await QuestionnaireHelper.list(null, { course: courseId, courseTimeline: END_COURSE });
